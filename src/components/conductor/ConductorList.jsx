...
export function ConductorList({ conductores, fetchNextPage, hasNextPage, handleEdit, deleteMutation, handleAscOrder, ascOrder }) {
  ...
  return (
    <InfiniteScroll
      ...
      next={() => {
        // guard and log to help debugging when next is called
        try {
          console.log('InfiniteScroll: requesting next page, hasNextPage=', hasNextPage)
          if (typeof fetchNextPage === 'function') fetchNextPage()
        } catch (err) {
          console.error('Error calling fetchNextPage', err)
        }
      }}
      hasMore={hasNextPage}
